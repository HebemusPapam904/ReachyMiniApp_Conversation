...
from conversation_relay.relay import send_transcript_webhook
from conversation_relay.config import (
    HF_BACKEND,
    LOCKED_PROFILE,
...
from conversation_relay.streaming import AdditionalOutputs, audio_to_float32
from conversation_relay.startup_settings import read_startup_settings, write_startup_settings
from conversation_relay.tools.core_tools import initialize_tools
from conversation_relay.personality_routes import (
    build_personality_ops,
    register_personality_methods,
)
from conversation_relay.audio.startup_config import apply_audio_startup_config
from conversation_relay.conversation_handler import ConversationHandler
...
# Windows' registry sometimes maps .js to text/plain, which browsers reject for
# <script type="module">. Force the correct type regardless of host config.
...
class LocalStream:
    ...
    def _dispatch_transcript(self, role: str, text: str, final: bool) -> None:
        """Push a conversation.transcript notification to JSON-RPC clients and relay it."""
        if self._rpc is not None:
            self._rpc.broadcast_threadsafe(
                "conversation.transcript",
                {"role": role, "text": text, "final": final},
            )
        send_transcript_webhook(role, text, final)
    ...
    def _persist_personality(self, profile: Optional[str], voice_override: Optional[str] = None) -> None:
        ...
        try:
            from conversation_relay.config import set_custom_profile
    ...
    async def apply_personality(self, profile: Optional[str]) -> str:
        ...
        try:
            from conversation_relay.config import set_custom_profile
            from conversation_relay.prompts import get_session_voice, get_session_instructions
    ...
    def get_current_voice(self) -> str:
        ...
        try:
            from conversation_relay.prompts import get_session_voice
