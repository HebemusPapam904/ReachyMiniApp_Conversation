...
from conversation_relay.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from conversation_relay.config import config
...
from conversation_relay.RAG.rag_tool import ChercherInfoEntreprise

if TYPE_CHECKING:
    from conversation_relay.mcp_client import RemoteMcpToolClient
    from conversation_relay.tools.background_tool_manager import BackgroundToolManager
...
def _load_profile_tools(tool_names: list[str], remote_tool_names: set[str]) -> List[type[Tool]]:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"conversation_relay.tools.{tool_name}"
            try:
