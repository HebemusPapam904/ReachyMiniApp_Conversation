...
from conversation_relay.config import HF_AVAILABLE_VOICES, config
from conversation_relay.console import LocalStream
from conversation_relay.startup_settings import (
    StartupSettings,
...
def test_backend_startup_failure_is_recorded_without_raising(
    ...
    monkeypatch.setattr("conversation_relay.console.apply_audio_startup_config", MagicMock())
...
def test_media_warmup_overlaps_audio_startup_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("conversation_relay.console.has_hf_realtime_target", lambda: True)
    ...
    monkeypatch.setattr("conversation_relay.console.asyncio.sleep", wait_for_audio_config)
    monkeypatch.setattr("conversation_relay.console.apply_audio_startup_config", apply_audio_config)
...
async def test_apply_personality_propagates_restart_cancellation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("conversation_relay.config.set_custom_profile", lambda _profile: None)
    monkeypatch.setattr(
        "conversation_relay.prompts.get_session_instructions", lambda _instance_path=None: "instructions"
    )
    monkeypatch.setattr("conversation_relay.prompts.get_session_voice", lambda default: default)
...
def test_local_stream_persist_personality_clears_legacy_startup_env_overrides(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "conversation_relay.config.set_custom_profile",
...
def test_local_stream_launch_waits_for_missing_hf_target_without_starting_media(
    ...
    monkeypatch.setattr("conversation_relay.console.time.sleep", MagicMock(side_effect=KeyboardInterrupt))
